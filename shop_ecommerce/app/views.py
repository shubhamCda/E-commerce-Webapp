...
def show_cart(request):
 if request.user.is_authenticated:
  ...
  totalamount = 0.0
  cart_product = [p for p in Cart.objects.all() if p.user == user] # list coprehension check the every cart obj. to if condition
  if cart_product:
...
class CustomerRegistrationView(View):
 ...
 def post(self, request):
  ...
  if form.is_valid():
   messages.success(request, 'Your details registered successfully!!')
   form.save()
  return render(request, 'app/customerregistration.html',{
   'form':form
  })
